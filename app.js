...
(function() {
  var ENEM_END, ENEM_RES_HEAD, Evernote, MIME_TO_EXTESION_MAPPING, argv, async, creatImportNote, createENEM_HEAD, createENEM_RES_END, createEmailNote, createHashHex, crypto, email, exec, filterImg, fs, fse, gimgFiles, limit, mime, noteStore, pwd, readImg, shell, sliceImg;
  ...
  filterImg = function(dir, limit) {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      ...
      if (type in MIME_TO_EXTESION_MAPPING && fs.statSync(file).size < 1024 * 1024 * limit) {
        ...
        tmp = dir + '/' + f;
        console.log(tmp);
        _results.push(filterImg(tmp, limit));
      } else {
    ...
    return async.auto({
      ...
      cImportNote: [
        'filter', function(cb, result) {
          ...
          return creatImportNote(filter, cb);
        }
      ]
    });
  };
